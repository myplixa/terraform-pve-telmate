locals { // Creating a pool and tags for grouping virtual machines
...
locals {
  data_disks = (
    lookup(var.disk, "data_sizes", "") == "" ? [] :
    compact([
      for d in split(",", lookup(var.disk, "data_sizes", "")) : trimspace(d)
    ])
  )
...
locals { // cloud-init configuration
  ...
  ssh_public_key = (local.ssh_user_key_file != null ?
    (
